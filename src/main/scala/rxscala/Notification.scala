...
object Notification {
  ...
  def wrap[T](n: rx.Notification[_ <: T]): Notification[T] = n.getKind match {
  ...
  object OnNext {
    ...
    def apply[T](value: T): Notification[T] = {
      Notification.wrap(new rx.Notification[T](value))
    }
  ...
  object OnError {
    ...
    def apply[T](error: Throwable): Notification[T] = {
      Notification.wrap(new rx.Notification[T](error))
    }
  ...
  object OnCompleted {
    ...
    def apply[T](): Notification[T] = {
      Notification.wrap(new rx.Notification())
    }
