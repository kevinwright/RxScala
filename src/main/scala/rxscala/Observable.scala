...
trait Observable[+T]
{
   ...
   */
  def multicast[R](subject: rxscala.Subject[T, R]): (() => Subscription, Observable[R]) = {
  ...
  def materialize: Observable[Notification[T]] = {
    Observable[rx.Notification[_ <: T]](asJavaObservable.materialize()).map(Notification wrap _)
  }
